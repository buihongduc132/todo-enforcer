...
- **New**: Config option `respectProgressAutoClear: boolean` (default `true`). When `todo-progress` clears its widget on a normal agent_end, `todo-enforcer` treats this as "all tasks resolved" and suppresses injection for that turn — but polling timer re-evaluates after cooldown.
- **New**: Config option `injectTodoPolicy: boolean` (default `false`). When `false`, `todo-enforcer` does NOT inject any system-prompt todo policy text (avoids double-policy when `todo-progress` is active). When `true`, injects a minimal enforcer-specific policy.
